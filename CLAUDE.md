...
agent_run.json     سجل الجلسة الحالية     <- سبب الخروج · آخر مرحلة · آخر مهمة
agent.log          اللوج
...
- ملفات `diag_*.py` و`diag_*.png` **أدوات تشخيص قديمة** — مش جزء من التشغيل.
  (الاستثناء: `diag_exitlog.py` جديد ومتغطّي تسجيل الخروج بـ15 فحص.)

**سبب الخروج بيتسجّل** (اتضاف 2026-09-08 بعد حادثة 16:33 اللي العملية اختفت
فيها من غير أثر): `agent_run.json` بيتكتب عند البداية بـ`clean=False` وبيتقفل
بـ`clean=True` عند أي خروج بيعدّي على بايثون، وأول حاجة في التشغيلة اللي بعدها
إنها تقرا الملف وتقول التشغيلة اللي فاتت خرجت إزاي:

| اللي في اللوج | معناه |
|---|---|
| «قفلت صح — السبب: …» | خروج معروف (إشارة إيقاف · استثناء · الحلقة وقفت) |
| «**ماكتبتش سطر خروج**» | العملية **اتقتلت من برّه** أو الجهاز اتقفل — مش عطل في الكود |

⚠️ `atexit` لوحده ماكانش هيكفي: القتل من برّه مابيشغّلوش أصلًا. الملف هو اللي
بيخلّي **الصمت نفسه دليل**.
