...
function buildReply(bc, row, journey) {
  ...
  if (journey?.err === 'pending') {
    ...
    // المدير 2026-09-06 (تاني): «عايز أشوف لحد تم تسجيل الطلب» — الميزانية
    // بقت أقصى ما يسمح به تليجرام (4096) ناقص سطر «جاري البحث» وهامش أمان.
    // رحلة 32 حالة ≈ 3550 حرف فبتظهر كاملة؛ القطع بس فوق ~38 حالة.
    const BUDGET = 3850;
    let used = L.join('\n').length;
    let shown = 0;
    for (const r of recs) {
